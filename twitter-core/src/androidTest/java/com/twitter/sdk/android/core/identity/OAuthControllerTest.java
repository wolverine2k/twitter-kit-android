...
public class OAuthControllerTest extends TwitterAndroidTestCase {
    ...
    public void testNewRequestTempTokenCallback_success() {
        ...
        callback.success(new Result<OAuthResponse>(oAuthResponse, null));
    ...
    public void testNewAccessTokenCallback_success() {
        ...
        final OAuthResponse oAuthResponse = new OAuthResponse(
                new TwitterAuthToken(TOKEN, SECRET), USER, USER_ID);
        callback.success(new Result<OAuthResponse>(oAuthResponse, null));

        final ArgumentCaptor<Intent> intentArgCaptor = ArgumentCaptor.forClass(Intent.class);
        verify(mockListener).onComplete(eq(Activity.RESULT_OK), intentArgCaptor.capture());
        final Intent data = intentArgCaptor.getValue();
        assertEquals(USER, data.getStringExtra(AuthHandler.EXTRA_SCREEN_NAME));
        assertEquals(USER_ID, data.getLongExtra(AuthHandler.EXTRA_USER_ID, 0L));
        assertEquals(TOKEN, data.getStringExtra(AuthHandler.EXTRA_TOKEN));
        assertEquals(SECRET, data.getStringExtra(AuthHandler.EXTRA_TOKEN_SECRET));
    }
