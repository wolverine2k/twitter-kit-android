...
public class SearchTimelineTest extends TweetUiTestCase {
    ...
    public void testFilterRetweets() {
        // FILTER_RETWEETS modifier should be added to the end of the search query by default
    ...
    // TODO: once timelines move to tweetui package, make Timeline use package scoped
    // AuthRequestQueue internally in order to switch from spies to mocks
    ...
    public void testCreateSearchRequest() {
        ...
        verify(mockSearchService).tweets(eq(TEST_QUERY + SearchTimeline.FILTER_RETWEETS),
                isNull(Geocode.class), eq(TEST_LANG), isNull(String.class), isNull(String.class),
                eq(TEST_ITEMS_PER_REQUEST), isNull(String.class), eq(TEST_SINCE_ID),
                eq(TEST_MAX_ID), eq(true), any(GuestCallback.class));
    }
