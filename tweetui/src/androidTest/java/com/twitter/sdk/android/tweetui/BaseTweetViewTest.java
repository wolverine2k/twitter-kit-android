...
public abstract class BaseTweetViewTest extends TweetUiTestCase {
    ...
    protected TweetViewMetric mockTweetViewMetric;
    ...
    protected void setUp() throws Exception {
        ...
        mockTweetViewMetric = mock(TweetViewMetric.class);
    ...
    private void setUpMockDependencyProvider() {
        ...
        when(mockDependencyProvider.getMetricsManager())
                .thenReturn(TweetUi.getInstance().getMetricsManager());
    ...
    // metrics delegation tests

    // construction should call init, start, and finishRender on the view's TweetViewMetric
    public void testTweetViewMetric_onInit() {
        createViewWithMocks(context, TestFixtures.TEST_TWEET);
        verify(mockTweetViewMetric, times(1)).start();
        verify(mockTweetViewMetric, times(1)).finishRender();
    }

    // setTweet should call start and finishRender on the view's TweetViewMetric
    public void testTweetViewMetric_onSetTweet() {
        final BaseTweetView view = createViewWithMocks(context, TestFixtures.TEST_TWEET);
        view.setTweet(TestFixtures.TEST_TWEET);
        // 1 start and finishRender from construction
        verify(mockTweetViewMetric, times(2)).start();
        verify(mockTweetViewMetric, times(2)).finishRender();
    }
